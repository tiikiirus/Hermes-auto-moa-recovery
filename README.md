...
| Pay (тяжёлые задачи) | `pay_auto_moa` | 5 pay-пресетов | gpt-5.6-luna-pro (агрегатор всех pay-пресетов) |
...
риск схемы. Платный агрегатор с 07.09.2026: `openai/gpt-5.6-luna-pro` ($0.20/$1.20 за 1M,
1.05M ctx; тот же тариф, что у базовой `luna` — pro отличается режимом
`reasoning.mode=pro`, выше качество рассуждений, расход thinking-токенов и
задержка) — выбран как компромисс «сильно и недорого»: в разы дешевле
