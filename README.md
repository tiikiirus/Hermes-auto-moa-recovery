...
| `skills/visual-explainer/` | Вендор скилла [visual-explainer](https://github.com/nicobailon/visual-explainer) (rev `7163c3e`, MIT): HTML/Mermaid вместо ASCII — диаграммы, diff-review, plan-review, слайды, таблицы. Проверено: `node skills/visual-explainer/quick/render.mjs spec.json out.html` собирает страницу из валидного спека (невалидный отклоняется валидатором). Установка в агент: `cp -R skills/visual-explainer ~/.config/opencode/skill/visual-explainer`. |
...
| Pay (тяжёлые задачи) | `pay_auto_moa` | 5 pay-пресетов | z-ai/glm-5.3-flash (агрегатор всех pay-пресетов) |
...
риск схемы. Платный агрегатор pay-ветки: `z-ai/glm-5.3-flash`
($0.06/$0.20 за 1M, 1.31M ctx, vision) — компромисс «достаточно сильно и
очень дёшево»: в десятки раз дешевле флагманов (Sonnet-5 $1.60/$8.00,
glm-5.3 non-flash $0.90/$2.82). Дорогой `openai/gpt-5.6-luna-pro`
($0.20/$1.20, reasoning=pro) откатён 07.09.2026: итоговая стоимость ходов
оказалась неприемлемой. Решение о подписке — за владельцем; без неё схема
...
python tools/moa_sync.py --sync   # backup + атомарная синхронизация
```

`--sync` сохраняет резервные копии в `%LOCALAPPDATA%\\hermes\\backups\\moa-sync-*`,
