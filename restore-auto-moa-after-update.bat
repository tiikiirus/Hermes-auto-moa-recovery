...
if errorlevel 1 (
  ...
  copy /y "!CONFIG_PATH!" "!CONFIG_PATH!.before-auto-moa-restore-%RANDOM%.bak" >nul || goto :config_backup_failed
  "!PYTHON!" -c "import sys,yaml,os; from pathlib import Path; p=Path(sys.argv[1]); b=Path(sys.argv[2]); d=yaml.safe_load(p.read_text(encoding='utf-8')) or {}; s=yaml.safe_load(b.read_text(encoding='utf-8')) or {}; keep=(d.get('moa') or {}).get('default_preset'); bd=(s.get('moa') or {}).get('default_preset'); d['moa']=s['moa']; d['moa']['default_preset']=keep or bd; print('[auto_moa] kept profile default_preset=%s (backup has %s)' % (d['moa'].get('default_preset'), bd)) if keep and keep != bd else None; t=str(p)+'.restore-tmp'; Path(t).write_bytes(yaml.safe_dump(d,sort_keys=False,allow_unicode=True).encode('utf-8')); os.replace(t,str(p))" "!CONFIG_PATH!" "%MOA_BACKUP%" || goto :config_restore_failed
) else (
